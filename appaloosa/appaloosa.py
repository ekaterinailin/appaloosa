...
def MultiFind(time, flux, error, flags, mode=5,
    ...
    if debug is True:
        relf = (flux-flux_model)/flux_model
        plt.figure(figsize=(7,5))
       # plt.title('debugging plot')
        plt.scatter(time, relf, alpha=0.5,label='flux',s=10)
        plt.plot(time,np.zeros_like(time), c='black',label='flux model')
        plt.scatter(time[cand1], relf[cand1], c='red',label='flare candidates',s=10)
        plt.xlabel('Time (BJD - 2454833 days)')
        #plt.ylabel(r'Flux (e- sec$^{-1}$)')
        plt.ylabel('rel. flux - 1')
        plt.legend()
        #plt.show()
        plt.savefig('debugging_plot.png',dpi=300)
        #plt.close()
...
def FakeFlaresDist(objectid, std, nfake, ampl=(5e-1,5e0), dur=(5e-1,2e1),
    ...
    if scatter == True:
        ...
        #plt.show()
        #plt.savefig(mode+'_fake_scatter.png',dpi=300)
    
        
    return dur_fake, ampl_fake, fig, ax, ax1    


def FakeFlares(objectid, time, flux, error, flags, tstart, tstop,
    ...
    dur_fake, ampl_fake, fig, ax, ax1 = FakeFlaresDist(objectid, std, nfake, mode='hawley2014', debug=debug, scatter=True)
    ...
    if display is True:
        print(str(nfake)+' flare candidates injected')
        isfake = np.where(allfl>1e-3)

        ax1.scatter(time, new_flux,s=3)
        ax1.scatter(time[isfake],new_flux[isfake],color='r',s=3)
...
#                        new_flux[istart[g]:istop[g]+1],color='red', lw=1)

    #        plt.plot(time, flux_model, 'blue', lw=3, alpha=0.7)
    #
    #        plt.plot(time, flux_model+error,'green')
    #        plt.plot(time, flux_model-error,'green')
        ax1.set_xlabel('Time (BJD - 2454833 days)')
        ax1.set_ylabel(r'rel. flux - 1')


        plt.savefig(objectid + '_lightcurve_inject.png', dpi=300, bbox_inches='tight')
...
#[dl[i]:dr[i]]
    ...
    if dofake is True:
        ...
        for k in range(iterations):
            print('Fake flare injection run {}'.format(k))
            for l,r in list(zip(dl,dr)):
                ...
                _ = pd.DataFrame()
                dur_fake, ampl_fake, _['ed_fake'], _['rec_fake'], ed_rec, ed_rec_err, istart_rec, istop_rec = FakeFlares(objectid, df2t.time,df2t.flux_gap/medflux - 1.0,
...
#        plt.plot(time, flux_model-error,'green')
        ...
        plt.savefig(file + '_lightcurve.png', dpi=300, bbox_inches='tight')
        plt.show()
